...
tf.flags.DEFINE_string("train_image_file", "Flickr8k_text\Flickr_8k.trainImages.txt",
                       "Training image file.")
tf.flags.DEFINE_string("val_image_file", "Flickr8k_text\Flickr_8k.devImages.txt",
                       "Validation image file.")
tf.flags.DEFINE_string("test_image_file", "Flickr8k_text\Flickr_8k.testImages.txt",
                       "Testing image file.")
tf.flags.DEFINE_string("captions_file", "Flickr8k_text\Flickr8k.token.txt",
                       "Testing image file.")

tf.flags.DEFINE_string("output_dir", "data\output2", "Output data directory.")
...
def _load_and_process_metadata(image_file,captions_file, image_dir="Flickr8k_Dataset"):
  ...
  """
  with tf.gfile.FastGFile(image_file,"r") as ff:
  ...
  print("Finished processing %d captions for %d images in %s" %
        (num_captions, len(filenames), captions_file))
...
def main(unused_argv):
  ...
  if not tf.gfile.IsDirectory(FLAGS.output_dir):
    tf.gfile.MakeDirs(FLAGS.output_dir) #判断是否有输出文件夹

  # Load image metadata from caption files.
  train_dataset = _load_and_process_metadata(FLAGS.train_image_file,
                                                    FLAGS.captions_file)
  val_dataset = _load_and_process_metadata(FLAGS.val_image_file,
                                                  FLAGS.captions_file)
  test_dataset = _load_and_process_metadata(FLAGS.test_image_file,
                                                  FLAGS.captions_file)
